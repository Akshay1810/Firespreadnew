...
class BaseModel(pl.LightningModule, ABC):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        size=(320, 224),  # Target spatial dimensions
        mode="bilinear",
        align_corners=False
    )
    ...
    def get_pred_and_gt(self, batch):
        ...
        if y_hat.shape[-2:] != y.shape[-2:]:
            y_hat = F.interpolate(y_hat.unsqueeze(1), size=y.shape[-2:], mode="bilinear", align_corners=False).squeeze(1)
    ...
    def compute_loss(self, y_hat, y):
        if self.hparams.loss_function == "Focal":
            ...
            # Suppose target is of shape [23, 128, 128] with values 0 and 1.
            # y_onehot = F.one_hot(y, num_classes=2)  # now shape: [23, 128, 128, 2]
            # y_onehot = y_onehot.permute(0, 3, 1, 2).float()  # reshape to [23, 2, 128, 128]
